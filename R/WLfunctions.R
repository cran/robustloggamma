...
gkl <- function(x, tau) {
  ...
  return(x) }
pwd <- function(x, tau) {
 if(tau==Inf) x <- log(x+1)
 else x <- tau*((x + 1)^(1/tau) - 1)
 return(x) }
...
Disparity.WML.loggamma <- function(y,mu0,sig0,lam0,lam.low,lam.sup,tol=0.001,maxit=100,lstep=TRUE,sigstep=TRUE,bw=0.3,raf="SCHI2",tau=0.5,nmod=1000){
