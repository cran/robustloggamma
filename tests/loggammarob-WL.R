...
if (!do.test) {
  ...
  set.seed(2345)
  WL <- loggammarob(x, method="WL", control=loggammarob.control(method="WL", lower=0, upper=2, n=30))
  WL$call <- NULL
  ...
  x <- sort(rloggamma(n=80, lambda=1))
  set.seed(2345)
  WLTest <- loggammarob(x, method="WL", control=loggammarob.control(method="WL", lower=0, upper=2, n=30))
  WLTest$call <- NULL  
  ...
  stopifnot(
    all.equal(WLTest, WL, tol = 2e-7)
  )
  stopifnot(
    all.equal(sumWLTest, sumWL, tol = 2e-7)
  )
}
